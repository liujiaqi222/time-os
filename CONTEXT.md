...
| -------------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `services/session`         | Session 状态机:get/start/pause/resume/finish/cancel/finishSessionReview/note             | Session 生命周期、事务、`session:`/`sessions:running` 锁;startSession 经幂等模块执行                                             |
| `services/distraction`     | 4 个方法                                                                                 | Distraction 读写;唯一实现「无 sessionId 时默认打到当前运行中 Session」的目标解析规则                                             |
| `services/dashboard`       | `getDashboard`                                                                           | Today 读模型;组合 Session 与 Settings 的 interface 取 active session 和 timezone/selectedTrackId,不直查 appSettings              |
